...
const app = express();
const port = 3000;
const masterKey = process.env.MASTA_KEY;
...
app.get("/filter", (req, res) => {
  ...
  for( let i = 0; i < jokes.length; i++ ){
    if( type === jokes[i].jokeType )
      sameTypeJokes.push(jokes[i]);
  }

  if( sameTypeJokes.length === 0 )
    res.json("No such type exists");
  else
    res.json(sameTypeJokes);
...
app.put("/jokes/:id", (req, res) => {
  ...
  for( i = 0; i < jokes.length; i++ ){
    ...
    if( id === jokes[i].id ){
      ...
      jokes[i].jokeText = req.body.jokeText;
      jokes[i].jokeType = req.body.jokeType;
      break;
    }
  }
...
app.patch("/jokes/:id", (req, res) => {
  ...
  for( i = 0; i < jokes.length; i++ ){
    ...
    if( id === jokes[i].id ){
      ...
      if( req.body.hasOwnProperty('jokeType') )
        jokes[i].jokeType = req.body.jokeType;
      if( req.body.hasOwnProperty('jokeText') )
        jokes[i].jokeText = req.body.jokeText;
      break;
    }
  }
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if( flag === false )
    res.json(`Joke with id: ${id} not found. No jokes were deleted.`);
  else
    res.send("OK");
...
app.delete("/all", (req, res) => {
  ...
  if( req.query.mastaKey === masterKey ){

    jokes = [];
    res.sendStatus(200);

  } else
    res.json("MastaKey provided is not valid");
